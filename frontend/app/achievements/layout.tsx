...
import {
  ...
  Network, Trophy, BarChart3, Users, Settings, LogOut, Zap, Gem, UsersRound
} from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { href: '/multiplayer', icon: UsersRound, label: 'Multiplayer', badge: 'NEW' },
          ...
          {filteredNav.map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                {item.label}
                {(item as { badge?: string }).badge && !active && (
                  <span className="ml-auto px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-glow/20 text-amber-light border border-amber-glow/30">{(item as { badge?: string }).badge}</span>
                )}
